...
char char_seven[30]; // 剩余热量调整

char char_eight[30]; // 自瞄模式

char char_nine[30]; // 判断头的方向
/**
  * @brief 	绘制直线
  * @param  Graphic 图形数据
						Graphic_Name 图形名
...
int last_heat_control = 0;
// 显示UI界面图形
void Show_UI()
{
	// 机器人ID设置
	if (Referee_Inf.game_robot_state.robot_id < 100)
	{
		Sendor_ID = Referee_Inf.game_robot_state.robot_id;
		Receiver_ID = 0x100 + Referee_Inf.game_robot_state.robot_id;
	}
	else if (Referee_Inf.game_robot_state.robot_id > 100)
	{
		Sendor_ID = Referee_Inf.game_robot_state.robot_id;
		Receiver_ID = 0x0164 + (Referee_Inf.game_robot_state.robot_id - 100);
	}
	...
	if (Send_Once_Flag == 0)
	{
		...
		// 将字符图形数据初始化
		Draw_Char(&character_one, "001", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_one, 12, 1, 80, 740);

		// 同上
		sprintf(char_two, "Gimbal Mode is Rotate %d", 1);
		Draw_Char(&character_two, "100", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_two, 12, 1, 80, 780);

		sprintf(char_three, "Cover");
		Draw_Char(&character_three, "101", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_three, 12, 1, 80, 660);
		...
		sprintf(char_five, "Fric Off");
		Draw_Char(&character_five, "103", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_five, 12, 1, 80, 820);
		...
		sprintf(char_seven, "Heat Remain %d", heat_control);
		Draw_Char(&character_seven, "105", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_seven, 12, 1, 80, 860);

		sprintf(char_eight, "Nuc Mode");
		Draw_Char(&character_eight, "106", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_eight, 12, 1, 80, 700);

		sprintf(char_nine, "Head");
		Draw_Char(&character_nine, "107", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, char_nine, 12, 1, 1680, 860);

		// 初始化整数数据
		Draw_Int(&interger_one, "301", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, Referee_Inf.power_heat_data.shooter_id1_42mm_cooling_heat, 12, 1, 80, 820);

		// 初始化浮点数数据
		Draw_Float(&float_one, "401", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, Referee_Inf.real_shoot_data.bullet_speed, 3, 12, 1, 80, 840);
		...
		Draw_Line(&line_four, "005", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, 1, SCREEN_LENGTH / 2 - 60, SCREEN_WIDTH / 2, SCREEN_LENGTH / 2 + 60, SCREEN_WIDTH / 2); // 横线
		...
		Draw_Line(&line_six, "007", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, 1, SCREEN_LENGTH / 2, SCREEN_WIDTH / 2 + 20, SCREEN_LENGTH / 2, SCREEN_WIDTH / 2 - 210); // 竖线
		...
		// 初始化圆形数据
		Draw_Circle(&circle_one, "200", Graphic_Operate_ADD, 9, Graphic_Color_Yellow, 1, SCREEN_LENGTH / 2, SCREEN_WIDTH / 2, 20);
		Draw_Circle(&circle_two, "201", Graphic_Operate_ADD, 9, Graphic_Color_Pink, 4, 200, 820, 15);
		Draw_Circle(&circle_three, "202", Graphic_Operate_ADD, 9, Graphic_Color_Pink, 4, 300, 740, 15);
		Draw_Circle(&circle_four, "203", Graphic_Operate_ADD, 9, Graphic_Color_Green, 4, 220, 660, 15);
	...
	{
		{
			sprintf(char_one, "Super Cap %.2f", SuperCap_Info.CapVot);
		}

		if (SuperCap_Info.CapVot < 19.0)
		{
			Draw_Char(&character_one, "001", Graphic_Operate_CHANGE, 9, Graphic_Color_Purplish_red, char_one, 12, 1, 80, 740);
		}
		else
		{
			Draw_Char(&character_one, "001", Graphic_Operate_CHANGE, 9, Graphic_Color_Green, char_one, 12, 1, 80, 740);
		}

		if (SuperCap_Info.CapVot < 20.0)
		{
			Draw_Circle(&circle_three, "202", Graphic_Operate_CHANGE, 9, Graphic_Color_Pink, 4, 300, 740, 15);
		}
		else
		{
			Draw_Circle(&circle_three, "202", Graphic_Operate_CHANGE, 9, Graphic_Color_Green, 4, 300, 740, 15);
		}
		...
		Draw_Char(&character_two, "100", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_two, 12, 1, 80, 780);
		...
		{
			sprintf(char_five, "Fric On");
			Draw_Circle(&circle_two, "201", Graphic_Operate_CHANGE, 9, Graphic_Color_Green, 4, 200, 820, 15);
			if (abs(CAN_Shoot[0].Current_Speed) < 10) // 摩擦轮堵住了
			{
				sprintf(char_five, "Fric Block");
			}
			Draw_Char(&character_five, "103", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_five, 12, 1, 80, 820);
		}
		else if (Shoot_Mode == SHOOT_MODE_STOP) // 摩擦轮关闭
		{
			sprintf(char_five, "Fric Off  ");
			Draw_Char(&character_five, "103", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_five, 12, 1, 80, 820);
			Draw_Circle(&circle_two, "201", Graphic_Operate_CHANGE, 9, Graphic_Color_Pink, 4, 200, 820, 15);
		}

		if (heat_control != last_heat_control) // 剩余热量改变
		{
			sprintf(char_seven, "Heat Remain %d", heat_control);
			Draw_Char(&character_seven, "105", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_seven, 12, 1, 80, 860);
			last_heat_control = heat_control;
		}

		if (NUC_tx_buff[8] == 0x83 || NUC_tx_buff[8] == 0x03) // 陀螺模式
		{
			sprintf(char_eight, "Nuc Mode Top");
			Draw_Char(&character_eight, "106", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_eight, 12, 1, 80, 700);
		}
		else if (NUC_tx_buff[8] == 0x80 || NUC_tx_buff[8] == 0x00) // 普通模式
		{
			sprintf(char_eight, "Nuc Mode Normal");
			Draw_Char(&character_eight, "106", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_eight, 12, 1, 80, 700);
		}
		else if (NUC_tx_buff[8] == 0x82 || NUC_tx_buff[8] == 0x02) // 打符模式
		{
			sprintf(char_eight, "Nuc Mode Dafu");
			Draw_Char(&character_eight, "106", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_eight, 12, 1, 80, 700);
		}

		if (CAN_Gimbal[0].Origin_MechAngle > 0 && CAN_Gimbal[0].Origin_MechAngle < 1500)
		{
			sprintf(char_nine, "Head is Back");
			Draw_Char(&character_nine, "107", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_nine, 12, 1, 1680, 860);
		}
		else
		{
			sprintf(char_nine, "Head is Front");
			Draw_Char(&character_nine, "107", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_nine, 12, 1, 1680, 860);
		}

		if (TIM1->CCR1 == COVER_OPEN_VAL)
		{
			sprintf(char_three, "Cover On");
			Draw_Char(&character_three, "101", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_three, 12, 1, 80, 660);
			Draw_Circle(&circle_four, "203", Graphic_Operate_CHANGE, 9, Graphic_Color_Pink, 4, 220, 660, 15);
		}
		else
		{
			sprintf(char_three, "Cover Off");
			Draw_Char(&character_three, "101", Graphic_Operate_CHANGE, 9, Graphic_Color_Yellow, char_three, 12, 1, 80, 660);
			Draw_Circle(&circle_four, "203", Graphic_Operate_CHANGE, 9, Graphic_Color_Green, 4, 220, 660, 15);
		}
